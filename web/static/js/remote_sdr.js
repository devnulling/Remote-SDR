...
var SDR_RX={
    Audio_RX:10489750000,
    fine:0,
    centrale_RX:10489750000,
    FrRX:0,
    offset:0,
    Ecart_LNB:0,
    idx_offset:0,
...
var voies_moy=new Array();

for (var i=0;i<FFT;i++) { v
    oies_moy[i]=0;
}
...
function Lance_Websocket_audio(){
    ...
    //Port serveur web de base +1 pour l'audio
    var adresse="ws://"+SDR_RX.IP+":"+(parseInt(SDR_RX.Port)+1).toString()+"/";
    ...
    websocket_audio.onmessage = function(evt) {
        ...
        reader.addEventListener("loadend", function() {
            ...
            if(deltaIndex>audioRX.Ctx.sampleRate*0.4) {
                audioRX.idx_charge = audioRX.idx_sortie + audioRX.Ctx.sampleRate * 0.19
            }

            audioRX.deltaIdx = audioRX.deltaIdx - deltaIndex / 100000000; //Coef correction

            if(deltaIndex<-audioRX.Ctx.sampleRate * 0.15){
                audioRX.idx_charge = audioRX.idx_sortie + audioRX.Ctx.sampleRate * 0.11;
            }
            ...
            if(audioTX.Transmit){
                Vol = SDR_RX.VolAudinTX; //Volume audio quand on transmet
            }
...
function choix_freq_central() {
    ...
    if (ZoomFreq.id=="DOF"){
        Affich_freq_champs(SDR_RX.Ecart_LNB,"#ZFr"); //Zoom display 
    }
}
...
function choix_mode(){ //MODE 
    ...
    if (web_socket.para_on  && web_socket.spectre_in){
        websocket_para.send('{"LSB_USB":"' + SDR_RX.mode + '"}');
    }
...
function choix_freq_fine(){  //Frequency for audio channel
    ...
    if (web_socket.para_on  && web_socket.spectre_in){
        websocket_para.send( '{"F_Fine":"' + SDR_RX.fine+'"}');
    }
...
function Choix_PB_RX(){ // Bandpass filter for the audio. Cannot be changed in GNU Radio to maintain 90° oscillator phases difference
    if(biquadFilterHP_RX != null){
        biquadFilterHP_RX.frequency.value = audioRX_PB.F1;
    }
    if(biquadFilterLP_RX != null){
        biquadFilterLP_RX.frequency.value = audioRX_PB.F2;
    }
...
function Affiche_Mode(){
    if (SDR_RX.mode==0){
        $("#Lsb").prop("checked",true); 
    }

    if (SDR_RX.mode==1){
        $("#Usb").prop("checked",true); 
    }
}
...
function Init_Page_RX(){
    ...
    if (SDR_RX.IP.length > 3){
        ...
        $.ajax({
            url: adresse, 
            datatype:"script", 
            success: function(result){ //Attention après cela les downloads de fichiers s'arretes
...
function ValideIP(){
    ...
    if (parseInt(V)<1025 || parseInt(V)>65535){
        V="";
    }
    ...
    if (V>1024){
        $("#RX_ports").html("used: "+V+","+(V+1)+","+(V+2)+","+(V+3));
    }
...
function WatchDog(){
    if(SDR_RX.IP.length>3) {
        ...
        if (Math.max(Watch_dog.RXpara,Watch_dog.RXspectre,Watch_dog.RXaudio)>5){
    ...
    if(SDR_TX.IP.length>3) {
        Watch_dog.TXpara++;
        if (Watch_dog.TXpara>6){
            $("#TXonLed").css("background-color","Red"); //Alerte messages n'arrivent pas
        }
    }
}
