...
function start_microphone(stream){
    ...
    audioTX.node_proces_sortie.onaudioprocess = function(audioProcessingEvent){
        ...
        if (websocket.audioTXpret && audioTX.Transmit){
            websocket.audioTX.send(OutData); //L'arret d'envoi des données coupe la puissance de l'emetteur
        }
        ...
        if (audioTX.cptFFT==0){ //On ne trace pas toutes les FFT
            ...
            if (Amp_max/audioTX.RC_Amp>0.2){
                couleur='Aqua'; 
            }
            if (Amp_max/audioTX.RC_Amp>0.7){
                couleur='orange'; 
            }
            if (Amp_max/audioTX.RC_Amp>0.98){
...
function  GainIF_TX(){
    if (audioTX.Transmit){
        if (websocket.paraTXpret){
            websocket.paraTX.send( '{"GIF_TX":"' + SDR_TX.GainIF+'"}'); //Gain IF
        }
    } else {
        if (websocket.paraTXpret){
...
function  GainBB_TX(){
    if (websocket.paraTXpret){
        websocket.paraTX.send( '{"GBB_TX":"' + SDR_TX.GainBB+'"}'); //Gain RF
    }
}
...
function  Mode_TX(){
    ...
    if (websocket.paraTXpret){
        websocket.paraTX.send( '{"LSB_USB":"' + M+'"}'); //-1 ou +1
    }
}
...
function Init_Page_TX(){
    ...
    if(SDR_TX.IP.length>3){
        ...
        $.ajax({
            url: adresse, 
            datatype:"script", 
            success: function(result){ //Attention ! After that the files download stop
...
function Recal_OFT(deltaF){
    ...
    if (ZoomFreq.id=="OFT"){
        Affich_freq_champs(SDR_TX.Offset,"#ZFr"); //Zoom display
    } 
...
function recupere_ancien_parametresTX(){
    if (localStorage.getItem("Para_SDR_TX")!=null){ // On a d'anciens parametres en local
        ...
        if (audioParaTX.Compresse==0){
            $("#V_Manuel").prop("checked",true);
        }
    
        if (audioParaTX.Compresse==1){
            $("#V_Auto1").prop("checked",true);
        }
    
        if (audioParaTX.Compresse==2){
            $("#V_Auto2").prop("checked",true);
        }
